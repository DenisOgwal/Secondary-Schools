        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (cmbBranch1.Text == "A Level")
                {
                    ...
                    myConnection = new SqlConnection(cs.DBConn);
                    MyCommand.Connection = myConnection;
                    MyCommand.CommandText = "select * from Attendance,Student where Student.Scholarno=Attendance.scholarNo and Attendance.Course= '" + cmbCourse1.Text + "' and Attendance.branch='" + cmbBranch1.Text + "' and Attendance.Session='" + cmbSession1.Text + "' and Attendance.Semester = '" + cmbSemester1.Text + "' and Attendance.Section = '" + cmbSection1.Text + "' and SubjectCodeA = '" + cmbSubjectCode.Text + "' and Status = 'Yes' and AttendanceDate between @date1 and @date2 order by Student.Student_name,Student.scholarNo ";
                    MyCommand.Parameters.Add("@date1", SqlDbType.DateTime, 30, " AttendanceDate").Value = dateTimePicker4.Value.Date;
                    MyCommand.Parameters.Add("@date2", SqlDbType.DateTime, 30, " AttendanceDate").Value = dateTimePicker3.Value.Date;
                    MyCommand.CommandType = CommandType.Text;
                    myDA.SelectCommand = MyCommand;
                    ...
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    cmd = new SqlCommand("select Count(ScholarNo) from Attendance where Course= '" + cmbCourse1.Text + "' and branch='" + cmbBranch1.Text + "' and Session='" + cmbSession1.Text + "' and Semester = '" + cmbSemester1.Text + "' and Section = '" + cmbSection1.Text + "' and SubjectCodeA = '" + cmbSubjectCode.Text + "' and AttendanceDate between @date1 and @date2 group by ScholarNo", con);
                    cmd.Parameters.Add("@date1", SqlDbType.DateTime, 30, " AttendanceDate").Value = dateTimePicker4.Value.Date;
                    cmd.Parameters.Add("@date2", SqlDbType.DateTime, 30, " AttendanceDate").Value = dateTimePicker3.Value.Date;
                    rdr = cmd.ExecuteReader();
        ...
        private void cmbSection1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbBranch1.Text == "A Level")
                {
                    ...
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    string ct = "select distinct RTRIM(SubjectCodeA) from SubjectInfoA where Class = '" + cmbCourse1.Text + "' and Level= '" + cmbBranch1.Text + "' and Term= '" + cmbSemester1.Text + "'";
                    cmd = new SqlCommand(ct);
                    cmd.Connection = con;
                    rdr = cmd.ExecuteReader();
                    while (rdr.Read())
        ...
        private void cmbSubjectCode_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbBranch1.Text == "A Level")
                {
                    ...
                    con.Open();
                    cmd = con.CreateCommand();
                    cmd.CommandText = "SELECT SubjectNameA FROM SubjectInfoA WHERE SubjectCodeA = '" + cmbSubjectCode.Text + "'";
                    rdr = cmd.ExecuteReader();
                    if (rdr.Read())
                    {
                        txtSubjectName.Text = rdr.GetString(0).Trim();
