...
{
    public partial class frmBalanceForward : Form
    {
        ...
        private void auto()
        {
            ...
            purchaseid.Text = "BF-" + years + monthss + days + GetUniqueKey(5);
        }
        ...
        private void delete_records()
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cq = "delete from BalanceForward where BalanceForwardID=@DELETE1;";
                cmd = new SqlCommand(cq);
                cmd.Connection = con;
                cmd.Parameters.Add(new SqlParameter("@DELETE1", System.Data.SqlDbType.NChar, 15, "BalanceForwardID"));
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string ct = "select BalanceForwardID from BalanceForward where  BalanceForwardID= '" + purchaseid.Text + "' ";
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string ct1 = "select BalanceForwardID from BalanceForward where BalanceForwardID=@find";
                cmd = new SqlCommand(ct1);
                cmd.Connection = con;
                cmd.Parameters.Add(new SqlParameter("@find", System.Data.SqlDbType.NChar, 15, "BalanceForwardID"));
                cmd.Parameters["@find"].Value = purchaseid.Text;
                rdr = cmd.ExecuteReader();
                if (rdr.Read())
                {
                    MessageBox.Show("Balance ID Already Exists", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    purchaseid.Text = "";
                    if ((rdr != null))
                    {
                        rdr.Close();
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cb = "insert into BalanceForward(BalanceForwardID,DateRegistered,Description,Price,Term,Year) VALUES (@d1,@d2,@d3,@d4,@d5,@d6)";
                cmd = new SqlCommand(cb);
                cmd.Connection = con;
                cmd.Parameters.Add(new SqlParameter("@d1", System.Data.SqlDbType.NChar, 15, "BalanceForwardID"));
                ...
                cmd.ExecuteNonQuery();
                MessageBox.Show("Successfully saved", "Balance Forward Record", MessageBoxButtons.OK, MessageBoxIcon.Information);
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void buttonX4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cb = "update BalanceForward set Price=@d4, BalanceForwardID=@d1,Description=@d3,DateRegistered=@d3 where BalanceForwardID=@d1";
                cmd = new SqlCommand(cb);
                cmd.Connection = con;
                cmd.Parameters.Add(new SqlParameter("@d1", System.Data.SqlDbType.NChar, 15, "BalanceForwardID"));
                cmd.Parameters.Add(new SqlParameter("@d2", System.Data.SqlDbType.NChar, 20, "DateRegistered"));
                ...
                cmd.Parameters["@d4"].Value = Price.Text;
                cmd.ExecuteNonQuery();
                MessageBox.Show("Successfully updated", "Balance Forward Record", MessageBoxButtons.OK, MessageBoxIcon.Information);
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        {
            this.Hide();
            frmBalanceForwardRecord frm = new frmBalanceForwardRecord();
