    ...
    public partial class frmAttendance : Form
    {
        ...
        public void AutocompleEmployeeID()
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string ct = "select distinct RTRIM(StaffID) from Employee where Designation='Teacher'";
                cmd = new SqlCommand(ct);
                cmd.Connection = con;
                rdr = cmd.ExecuteReader();
                while (rdr.Read())
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select ScholarNo,Enrollment_no,student_name from Student,batch where batch.Course=Student.Course and Student.Course = '" + cmbCourse.Text + "' and Student.Branch= '" + cmbBranch.Text + "' and Batch.semester= '" + cmbSemester.Text + "' and Batch.Session= '" + cmbSession.Text + "' and section='" + cmbSection.Text + "' order by student_name,ScholarNo", con);
                rdr = cmd.ExecuteReader();
                while (rdr.Read())
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (cmbSession.Text == "")
            {
             ...
             if (cmbCourse.Text == "")
                ...
                if (cmbSection.Text == "")
                {
                    MessageBox.Show("Please select Strem", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cmbSection.Focus();
                    return;
                }
            ...
            try 
            {
                for (int i = listView1.Items.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        con = new SqlConnection(cs.DBConn);
                        string cd = "insert into Attendance(course,branch,semester,session,section,subjectcodeA,subjectname,staffid,attendancedate,scholarno,Status) VALUES (@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8,@d9,@d10,@d11)";
                        cmd = new SqlCommand(cd);
                        cmd.Connection = con;
                        cmd.Parameters.AddWithValue("d1", cmbCourse.Text);
                        cmd.Parameters.AddWithValue("d2", cmbBranch.Text);
           ...
           private void cmbSection_SelectedIndexChanged(object sender, EventArgs e)
           {
               try
               {
                ...
                if (cmbBranch.Text == "A Level")
                {
                    ...
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    string ct = "select distinct RTRIM(SubjectCodeA) from SubjectInfoA where Class = '" + cmbCourse.Text + "' and Level= '" + cmbBranch.Text + "' and Term= '" + cmbSemester.Text + "'";
                    cmd = new SqlCommand(ct);
                    cmd.Connection = con;
                    rdr = cmd.ExecuteReader();
                    while (rdr.Read())
