    ...
    public partial class frmPurchasePayment : Form
    {
        ...
        private void Autocomleteids()
        {
            try
            {
                ...
                CN.Open();
                adp = new SqlDataAdapter();
                adp.SelectCommand = new SqlCommand("SELECT (purchaseid) FROM Purchases order by ID DESC", CN);
                ds = new DataSet("ds");
                adp.Fill(ds);
                dtable = ds.Tables[0];
                paidfor.Items.Clear();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                con.Open();
                string ct = "select Amount from PurchasePayment where  Reason= '" + paidfor.Text + "'";
                ...
                if (rdr.Read())
                {
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    string cb2 = "update PurchasePayment set DuePayment=@d8 where Reason=@d7";
                    ...
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    string cb = "insert into PurchasePayment(TransactionType,Date,Amount,Description,Term,Year,Reason,DuePayment) VALUES (@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8)";
        ...
        private void delete_records()
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cq = "delete from PurchasePayment where TransactionID=@DELETE1;";
        ...
        private void Update_record_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cb = "update PurchasePayment set TransactionType=@d1,Date=@d2,Amount=@d3,Description=@d4,Term=@d5,Year=@d6 where TransactionType=@d1  and Date=@d2 and Year=@d6 and Term=@d5";
                cmd = new SqlCommand(cb);
                cmd.Connection = con;
                cmd.Parameters.Add(new SqlParameter("@d1", System.Data.SqlDbType.NChar, 10, "TransactionType"));
                cmd.Parameters.Add(new SqlParameter("@d2", System.Data.SqlDbType.NChar, 30, "Date"));
        ...
        {
            this.Hide();
            frmpurchasepaymentrecord frm = new frmpurchasepaymentrecord();
        ...
        private void paidfor_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "SELECT totalcost FROM Purchases WHERE purchaseid = '" + paidfor.Text + "'";
                rdr = cmd.ExecuteReader();
                if (rdr.Read())
                {
        ...
        private void txtamt_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string ct = "select DuePayment from PurchasePayment where  Reason= '" + paidfor.Text + "' order by TransactionID Desc";
                cmd = new SqlCommand(ct);
                cmd.Connection = con;
                rdr = cmd.ExecuteReader();
                if (rdr.Read())
