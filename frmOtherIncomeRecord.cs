...
{
    public partial class frmOtherIncomeRecord : Form
    {   
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(TransactionID)[Transaction ID],RTRIM(TransactionType)[Transaction Type],RTRIM(Date)[Transaction Date],RTRIM(Amount)[Amount],RTRIM(Description)[Description] ,RTRIM(Reason)[Paid For],RTRIM(Year)[Year],RTRIM(Term)[Term]FROM OtherIncomes where date between @date1 and @date2 order by TransactionID Desc", con);
